...
const App: React.FC = () => {
  ...
  const renderLandingView = () => <div className="bg-[#030712] min-h-screen font-sans text-slate-100 selection:bg-blue-500/30">
    ...
    <footer className="bg-nexus-card border-t border-nexus-border py-12"><div className="container mx-auto px-6 text-center text-slate-500">
      <div className="max-w-xl mx-auto mb-10 rounded-2xl border border-emerald-500/15 bg-slate-950/60 p-6"><div className="flex flex-col sm:flex-row items-center justify-center gap-6"><a href={`${WHATSAPP_DIRECT_URL}?text=Hi%2C%20I%20want%20to%20discuss%20a%20managed%20AI%20system.`} target="_blank" rel="noopener noreferrer" className="shrink-0 rounded-xl bg-white p-3 shadow-lg" aria-label={`Scan QuickKit AI WhatsApp QR for @${WHATSAPP_USERNAME}`}><img src={WHATSAPP_QR_ASSET} alt={`QuickKit AI WhatsApp QR — @${WHATSAPP_USERNAME}`} className="w-36 h-36" /></a><div className="text-left"><p className="text-xs font-mono tracking-[0.2em] uppercase text-emerald-400 mb-2">Direct WhatsApp</p><h2 className="text-xl font-black text-white mb-1">@{WHATSAPP_USERNAME}</h2><p className="text-sm text-slate-400 mb-4">Scan the QR or message QuickKit AI directly.</p><div className="flex flex-wrap gap-2"><a href={`${WHATSAPP_DIRECT_URL}?text=Hi%2C%20I%20want%20to%20discuss%20a%20managed%20AI%20system.`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg bg-emerald-600 px-4 py-2 text-xs font-black text-white hover:bg-emerald-500 transition-colors"><MessageCircle className="w-4 h-4" /> Message Now</a><a href={WHATSAPP_QR_ASSET} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-slate-700 px-4 py-2 text-xs font-black text-slate-300 hover:bg-slate-800 transition-colors"><QrCode className="w-4 h-4" /> Open QR</a></div></div></div></div>
