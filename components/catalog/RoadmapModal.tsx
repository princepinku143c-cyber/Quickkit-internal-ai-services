 ...
 * The public payment flow is Razorpay-only; this component no longer embeds PayPal.
 * It is kept as a lightweight compatibility route for older imports.
 */
export const RoadmapModal: React.FC<RoadmapModalProps> = ({ item, customPrompt, onClose, onBook }) => {
  ...
  const handleContinue = () => {
    ...
    onBook?.({
      service: item?.name || 'Custom AI System',
      setup: item?.setupUSD || 0,
      monthly: item?.monthlyUSD || 0,
      currency: 'INR' as any,
      notes: requirement,
    } as AIQuote, history);
  };
      ...
      <div className="w-full max-w-2xl rounded-3xl border border-slate-800 bg-[#080c14] shadow-2xl overflow-hidden">
        ...
        <div className="p-6 md:p-8 space-y-6">
          ...
          <textarea
            ...
            placeholder="What should your AI system do?"
            className="w-full min-h-36 rounded-2xl bg-slate-950 border border-slate-800 text-white p-4 outline-none focus:border-blue-500/50 resize-y"
          />

          <button onClick={handleContinue} className="w-full py-4 rounded-2xl bg-white text-slate-900 font-black uppercase tracking-widest text-sm flex items-center justify-center gap-2 hover:bg-slate-100 transition-colors">
