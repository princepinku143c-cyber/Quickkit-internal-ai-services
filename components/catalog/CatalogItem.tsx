...
export const CatalogItem: React.FC<CatalogItemProps> = ({ item, currency, onSelect }) => {
  ...
  return (
    ...
    >
      ...
      <h4 className="font-semibold text-white mb-4 pr-6 group-hover:text-cyan-300 transition-colors line-clamp-2">
        {item.name}
      </h4>
      ...
      <div className="flex items-center justify-between mt-auto">
        <div className="text-xs text-slate-400">
          ...
          <p className="text-lg font-bold text-white">
            {currency === 'INR' ? `₹${item.setupINR.toLocaleString()}` : `$${item.setupUSD}`}
          </p>
        </div>
        ...
        <div className="text-xs text-right">
          ...
          <p className="text-lg font-bold text-emerald-400">
            {currency === 'INR' 
              ? `₹${item.monthlyINR.toLocaleString()}` 
              : `$${item.monthlyUSD}`}
          </p>
        </div>
      </div>
