...
export const FloatingChatWidget: React.FC = () => {
        ...
        try {
          ...
          fetch('/api/system?action=lead', {
            ...
            body: JSON.stringify({
              name: 'Chat Visitor',
              email: email,
              requirement: 'Lead captured dynamically via Kelly AI Chat Widget consultation.',
              projectName: 'Chatbot Consult Inquiry'
            })
          });
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
      {/* Chat Window */}
      {isOpen && (
        <div className="w-80 sm:w-96 h-[500px] mb-4 bg-[#0f172a]/95 backdrop-blur-xl border border-[#1e293b] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-fade-in">
          {/* Header */}
          <div className="p-4 bg-gradient-to-r from-blue-600 to-indigo-600 flex items-center justify-between text-white shadow-lg">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h4 className="text-xs font-black uppercase tracking-widest flex items-center gap-1">Kelly <Sparkles className="w-3 h-3 text-yellow-300" /></h4>
                <p className="text-[9px] font-bold text-blue-200 uppercase tracking-wider">AI Operations Consultant</p>
              </div>
            </div>
            <button 
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 custom-scrollbar bg-[#050810]/40">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-[#1e293b]/70 border border-[#1e293b] text-slate-400 rounded-2xl rounded-tl-sm px-4 py-3 text-xs flex items-center gap-1.5 font-bold uppercase tracking-wider animate-pulse">
                  <Bot className="w-3.5 h-3.5 text-blue-400 animate-spin" />
                  <span>Kelly is writing...</span>
                </div>
              </div>
            )}
          ...
          <form 
            ...
            className="p-3 bg-slate-950/80 border-t border-[#1e293b]/40 flex gap-2"
          >
            <input
              type="text"
              value={input}
              ...
              className="flex-1 bg-[#050810] border border-[#1e293b] rounded-xl px-4 py-2.5 text-xs text-slate-200 outline-none focus:border-blue-500 transition-colors"
            />
            <button
              type="submit"
      ...
      <button
        ...
        className="p-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-full shadow-2xl hover:scale-105 transition-all shadow-blue-600/20 flex items-center justify-center relative group"
        title="Consult Kelly"
      >
